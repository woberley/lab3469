...
void ProcessSchedule () {
  #ifdef RR_SCHED
    ProcessScheduleRR();
  #elif LT_SCHED_STATIC
    ProcessScheduleLTS();
  #elif LT_SCHED_DYNAMIC
    ProcessScheduleLTD();
  #else
    printf ("NO SCHEDULER IS DEFINED!\n");
    exitsim ();
  #endif
}

void ProcessScheduleRR () {
  PCB *pcb=NULL;
  int i=0;
  Link *l=NULL;
  ...
  pcb->jiffieCount += ClkGetCurJiffies() - pcb->lastStarted;
  if (pcb->pinfo) {
    printf("CPUStats: Process %d has run for %d jiffies, priority = %d\n", GetCurrentPid(), pcb->jiffieCount, pcb->pnice);
  }
  currentPCB = pcb;
  ...
  pcb->lastStarted = ClkGetCurJiffies();
}

void ProcessScheduleLTS() {
  PCB *pcb=NULL;
  int i=0;
  Link *l=NULL;
  ...
  // GET THE NUMBER OF TIX IN THE READY QUEUE
  num_tix_in_rQueue = 0;
  l = AQueueFirst(&runQueue);
  while (l != NULL) {
    pcb = (PCB *)AQueueObject(l);
    num_tix_in_rQueue += pcb->pnice;
    l = AQueueNext(l); 
  }

  // INITIALIZE RAND AND BOUNDS
  rand_num = random() % num_tix_in_rQueue;
  starting_bound = 0;
  ending_bound = 0;

  // GO THROUGH RUN QUEUE AND PICK WINNER
  l = AQueueFirst(&runQueue);
  for (i=AQueueLength(&runQueue); i > 0; i--) {
    pcb = (PCB *)AQueueObject(l);
    ending_bound += (pcb->pnice)-1;
    if (rand_num >= starting_bound && rand_num <= ending_bound) {
      currentPCB = pcb;
      break;
    }
    starting_bound += ending_bound + 1;
    ending_bound += 1;
    l = AQueueNext(l);
  }
